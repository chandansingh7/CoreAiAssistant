...
import com.corecrew.coreaiassistant.service.SpeechToTextService;
...
public class AssistantController {
    ...
    @FXML private CodeArea codeInput;
    ...
    private void initialize() {
        ...
        // inside initialize(), after you startListening():
        Platform.runLater(() -> {
            Stage stage = (Stage) chatView.getScene().getWindow();
        ...
        // and as a fallback, in case of a crash or non‐JavaFX shutdown:
        ...
        String baseHtml = """
            ...
            <head>
                <style>
                    ...
                    .chat-container { display: flex; flex-direction: column; gap: 16px; padding: 16px; }
                    .message { max-width: 80%; padding: 12px 16px; border-radius: 12px; word-wrap: break-word; white-space: pre-wrap; }
                    .user { align-self: flex-end; background-color: #007aff; color: #fff; border-bottom-right-radius: 4px; }
                    .ai { align-self: flex-start; background-color: #f1f1f2; color: #000; border-bottom-left-radius: 4px; }
                    pre { background: #2d2d2d; color: #f8f8f2; padding: 10px; border-radius: 6px; overflow-x: auto; font-family: Consolas, monospace; white-space: pre-wrap; word-break: break-word; }
                    code { font-family: Consolas, monospace; background: rgba(0,0,0,0.05); padding: 2px 4px; border-radius: 4px; word-wrap: break-word; }
                </style>
            </head>
            <body>
            ...
            speechService.startListening(recognizedText -> {
                Platform.runLater(() -> {
                    if (!recognizedText.isBlank()) {
                        codeInput.replaceText(recognizedText);
                        onSendClicked();
                    }
                });
            });
